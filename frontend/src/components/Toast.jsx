...
const Toast = ({ notification, onClose, error, onErrorClose, message, type }) => {
    ...
    return (
        <div className="fixed bottom-6 left-1/2 -translate-x-1/2 z-50 pointer-events-auto flex flex-col items-center gap-2 max-w-[calc(100vw-2rem)] sm:max-w-md w-auto px-2 sm:px-4 select-none">
            {activeNotification && (
                <div
                    className={`flex items-center gap-2.5 sm:gap-3 px-3.5 sm:px-4 py-2 sm:py-2.5 rounded-xl border shadow-2xl backdrop-blur-xl transition-all duration-200 animate-slide-up-fade w-max max-w-full ${
                ...
                >
                    ...
                    <span className="text-xs sm:text-sm font-medium tracking-wide whitespace-nowrap truncate min-w-0">
                    ...
                    {onClose && (
                        <button
                            type="button"
                            onClick={onClose}
                            className="ml-1 p-1 text-gray-400 hover:text-white rounded-md hover:bg-white/10 transition-colors cursor-pointer shrink-0"
                            title="Dismiss"
                        >
            ...
            {error && (
                <div className="flex items-center gap-2.5 sm:gap-3 px-3.5 sm:px-4 py-2 sm:py-2.5 rounded-xl border border-rose-500/30 bg-[#0c1017]/95 text-gray-100 shadow-2xl shadow-rose-950/40 backdrop-blur-xl transition-all duration-200 animate-slide-up-fade w-max max-w-full">
                    <div className="w-5 h-5 rounded-full bg-rose-500/15 border border-rose-500/30 flex items-center justify-center shrink-0 text-rose-400 shadow-sm shadow-rose-500/20">
                        <svg className="w-3 h-3" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth={2.5} strokeLinecap="round" strokeLinejoin="round">
                            <circle cx="12" cy="12" r="10" />
                            <line x1="12" y1="8" x2="12" y2="12" />
