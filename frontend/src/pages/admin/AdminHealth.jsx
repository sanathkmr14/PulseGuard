...
const HealthCard = ({ title, status, details }) => (
    <div className="bg-slate-800/40 backdrop-blur-xl border border-slate-700/60 rounded-xl p-3.5 sm:p-4 shadow-sm">
        <div className="flex items-center justify-between mb-3">
            <h3 className="text-white font-semibold text-xs sm:text-sm">{title}</h3>
            <span className={`px-2 py-0.5 rounded-full text-[10px] font-semibold border ${status === 'Operational' || status === 'Connected' || status === 'Running'
            ...
            {details ? (
                Object.entries(details).map(([key, value]) => (
                    <div key={key} className="flex justify-between items-center text-xs">
                        <span className="text-slate-400 capitalize text-[11px]">{key.replace(/([A-Z])/g, ' $1').trim()}</span>
                        <span className="text-white font-mono text-[11px]">{formatValue(key, value)}</span>
                    </div>
                ))
            ) : (
        ...
        return (
            <div className="flex justify-center items-center h-64">
                <div className="flex items-center gap-3 text-slate-400">
                    ...
                    <span className="font-medium text-xs sm:text-sm">Connecting to infrastructure...</span>
                </div>
            </div>
        );
    }
    ...
    return (
        <div className="space-y-4 sm:space-y-5 animate-fade-in">
            <div>
                ...
                <p className="text-slate-400 text-xs sm:text-sm font-medium">Monitor infrastructure performance and status</p>
            </div>
            ...
            <div className="bg-amber-500/10 border border-amber-500/20 rounded-lg p-3">
                <div className="flex items-start gap-2.5">
                    ...
                    <div>
                        <h3 className="text-xs font-semibold text-amber-400">Maintenance Mode</h3>
