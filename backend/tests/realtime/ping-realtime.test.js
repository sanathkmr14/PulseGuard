#!/usr/bin/env node
/**
 * PING/ICMP PROTOCOL REAL-TIME TEST
 * Tests ICMP ping with real endpoints
 * 
 * Validates:
 * ✅ Host reachability (UP)
 * ✅ Host unreachability (DOWN)
...
const TEST_SCENARIOS = [
    ...
    {
        name: 'Unreachable Host - Private IP',
        url: '10.255.255.1',
        timeout: 3000,
        expected: { healthState: 'DOWN', errorType: 'PING_TIMEOUT' }
    },
    {
        name: 'Invalid IP - Broadcast',
        url: '255.255.255.255',
        timeout: 3000,
        expected: { healthState: 'DOWN', errorType: 'HOST_UNREACHABLE_PING' }
    },
    {
...
async function testPingScenario(scenario, attempt = 1) {
    try {
        ...
        if (attempt < 2) {
            ...
            return testPingScenario(scenario, attempt + 1);
        }
...
async function runAllTests() {
    console.log('\n' + '='.repeat(90));
    console.log('🧪 PING/ICMP PROTOCOL REAL-TIME TEST SUITE');
    console.log('='.repeat(90));
    console.log(`Testing ${TEST_SCENARIOS.length} PING scenarios with real endpoints`);
    console.log('='.repeat(90) + '\n');
    ...
    for (const scenario of TEST_SCENARIOS) {
        ...
        const testResult = await testPingScenario(scenario);
